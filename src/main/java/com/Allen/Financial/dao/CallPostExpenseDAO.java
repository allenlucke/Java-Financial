...
import main.java.com.Allen.Financial.bean.Expenses;
...
public class CallPostExpenseDAO {
	...
	public List<Expenses> callPostExpense(Expenses expense) throws ServletException {
		...
		try{
			...
			cs.setLong(AMOUNT_DUE_INDEX, expense.getAmount_due());
			cs.registerOutParameter(CURSOR_INDEX, OracleTypes.CURSOR);
			
			cs.execute();
			
			rs = (ResultSet) cs.getObject(7);
			List<Expenses> expenseResultList = new ArrayList<Expenses>();
			...
			while(rs.next()) {
			    ...
			    long amount_due = rs.getLong("amount_due");
